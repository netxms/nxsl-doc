...
NOTE: In older scripts, you may see an explicit `main()` function. This is no longer required - the interpreter automatically wraps code outside functions into an implicit `main()`. The explicit `main()` function is still supported for backward compatibility.
