...
For more complex scripts, define helper functions. Code outside functions is automatically executed (the interpreter wraps it into an implicit `main()`):
...
* `trace(level, text)` - Debug output (level 1-9), written to server debug console and log file if level is above current debug level
