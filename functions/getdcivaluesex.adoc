...
GetDCIValuesEx(node, dciId, startTime, endTime, rawValues): <<class-array,Array>>::
...
+
NOTE: The input parameters `startTime` and `endTime` accept timestamps in seconds, but the returned <<class-datapoint,DataPoint>> objects contain timestamps in milliseconds since epoch (as of version 6.0).
