...
method, when that method returns the integer zero or Boolean value `false`.
...
Inserts new element to the array at `index`. Indexes of existing elements that had index >= `index` are incremented. 
...
Inserts elements of `anotherArray` to the array at `index`. Indexes of existing elements that have index >= `index` are incremented. 
...
`remove(index) => void`::
Removes element at specified `index`. Indexes of elements that have index >= `index` are decremented. 
