...
This means variables defined in the main script are not visible to other functions:
...
[[string-fuzzyEquals]]
fuzzyEquals(string, threshold): bool::
Returns `true` if argument string is approximately equal to this string using fuzzy comparison based on <<func-similarityscore,similarity score>>. Optional `threshold` parameter (float, 0.0 to 1.0) sets the minimum similarity score required for a match — higher values require closer match. Default threshold is 0.2.
+
[.version-added]
Added in version 6.0
+
[.source]
....
println("kitten".fuzzyEquals("kittens"));      // prints "true"
println("abc".fuzzyEquals("xyz"));             // prints "false"
println("hello".fuzzyEquals("hallo", 0.9));    // prints "false" (higher threshold)
....

[[string-fuzzyEqualsIgnoreCase]]
fuzzyEqualsIgnoreCase(string, threshold): bool::
Same as <<string-fuzzyEquals,fuzzyEquals>>, but ignoring the case.
+
[.version-added]
Added in version 6.0
+
[.source]
....
println("Kitten".fuzzyEqualsIgnoreCase("kittens"));  // prints "true"
println("Hello".fuzzyEqualsIgnoreCase("HALLO"));     // prints "true"
....
...
left(numberOfCharacters, paddingCharacter): <<class-string,String>>::
...
replace(whatToReplace, replaceBy): <<class-string,String>>::
...
right(numberOfCharacters, paddingCharacter): <<class-string,String>>::
...
substring(position, numberOfCharacters): <<class-string,String>>::
...
toLowerCase(): <<class-string,String>>::
...
toUpperCase(): <<class-string,String>>::
...
An array in NXSL is an ordered, sparse, integer-indexed collection. It can be
treated as an array, list (vector), stack, queue, and probably more. Nested
arrays are supported, so elements of an array can be themselves arrays.
...
`indexOf(value): int`:: Returns the index of first occurrence of the
...
`join(separator): <<class-string,String>>`::
...
`pop(): any`::
Removes and returns element with highest index from the array. Using `push(value)` and `pop()` methods it's possible to use array as a stack. Or, using
