...
[[datacollectiontarget-deleteDCI]]
deleteDCI(dciId): bool::
Delete data collection item from this object. DCIs created from a template are managed by that template and cannot be deleted this way; use <<class-template>> methods or the template itself instead.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
...
+
.Return
`true` on success, `false` on failure (caller has no modify access to the object, DCI not found, or DCI was created from a template).
+
[.version-added]
Added in version 6.2
+
[.source]
....
dci = $node.findDCIByName("Temporary.Metric");
if (dci != null)
   $node.deleteDCI(dci.id);
....
