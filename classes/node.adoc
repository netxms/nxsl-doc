[.nxsl-class]
[[class-node]]
=== Node

Represents NetXMS node object. Extends <<class-datacollectiontarget>>.

==== Instance attributes

include::_shared/netobj-attrs.adoc[]

include::_shared/datacollectiontarget-attrs.adoc[]

[.attr-list]
--
[[node-agentCertificateFingerprint]]
agentCertificateFingerprint: <<class-string,String>>::
Fingerprint of the certificate issued for agent tunnel on this node, as hexadecimal string. Empty string if no certificate was issued.
+
[.version-added]
Added in version 7.0

[[node-agentCertificateMappingData]]
agentCertificateMappingData: <<class-string,String>>::
Agent certificate mapping data (set in object properties).

[[node-agentCertificateMappingMethod]]
agentCertificateMappingMethod: <<class-string,String>>::
Agent certificate mapping method (set in object properties).

[[node-agentCertificateSubject]]
agentCertificateSubject: <<class-string,String>>::
Subject of certificate issued for agent tunnel on this node.

[[node-agentId]]
agentId: <<class-string,String>>::
NetXMS agent unique ID (`string` representation of GUID). Will return all zeroes GUID if agent is not detected on node or does not have unique ID.

[[node-agentPlatformName]]
agentPlatformName: <<class-string,String>>::
Platform name of the NetXMS agent build running on this node (value of agent's `System.PlatformName` metric). May differ from <<node-platformName,platformName>> when, for example, a 32-bit agent runs on a 64-bit system.
+
[.version-added]
Added in version 6.2

[[node-agentProxy]]
agentProxy => <<class-node>>::
Object that is set as agent proxy in object properties or NULL.

[[node-agentTlsMode]]
agentTlsMode: int::
TLS mode configured for direct agent connections:
+
* 0: TLS disabled
* 1: TLS allowed (used if supported by agent)
* 2: TLS required
+
[.version-added]
Added in version 7.0

[[node-agentVersion]]
agentVersion: <<class-string,String>>::
NetXMS agent version as `string`.

[[node-bootTime]]
bootTime: int64::
UNIX time (number of seconds since epoch) of the moment when node was started or `0` if unknown.

[[node-bridgeBaseAddress]]
bridgeBaseAddress: <<class-string,String>>::
Base address of the switch formatted as 12 character `string` without separators. Value is only valid for bridges and switches. Special value `000000000000` indicates that address is unknown.

[[class-node-capabilities,Node::capabilities]]
capabilities: int::
Detected node capabilities ("Have Agent", "Support SNMP", etc.) Bitwise OR of <<const-node-capability>> constants.

[[node-cipDeviceType]]
cipDeviceType: int::
EtherNet/IP device type

[[node-cipDeviceTypeAsText]]
cipDeviceTypeAsText: <<class-string,String>>::
EtherNet/IP device type as text

[[node-cipExtendedStatus]]
cipExtendedStatus: int::
EtherNet/IP device extended status

[[node-cipExtendedStatusAsText]]
cipExtendedStatusAsText: <<class-string,String>>::
EtherNet/IP device extended status as text

[[node-cipState]]
cipState: int::
EtherNet/IP device state

[[node-cipStateAsText]]
cipStateAsText: <<class-string,String>>::
EtherNet/IP device state as text

[[node-cipStatus]]
cipStatus: int::
EtherNet/IP device status

[[node-cipStatusAsText]]
cipStatusAsText: <<class-string,String>>::
EtherNet/IP device status as text

[[node-cipVendorCode]]
cipVendorCode: int::
EtherNet/IP vendor code in numeric form.

[[node-cluster]]
cluster => <<class-cluster>>::
Cluster this node is in.

[[node-components]]
components => <<class-component>>::
Entity MIB components of class <<class-component>>.

[[node-dependentNodes]]
dependentNodes: <<class-array,Array>>::
Will return array with <<class-nodedependency>> class objects. This array contains all objects that have current node as a proxy or data collection sources.

[[node-downSince]]
downSince: int64::
UNIX timestamp when node went down.

[[node-driver]]
driver: <<class-string,String>>::
Named of selected device-specific SNMP driver.

[[node-expectedCapabilities]]
expectedCapabilities: int::
Expected capabilities bitmask for the node. Capabilities set in this mask are protected from automatic removal when protocols become temporarily unreachable. Bitwise OR of <<const-node-capability>> constants. See <<node-setExpectedCapabilities,setExpectedCapabilities()>>.
+
[.version-added]
Added in version 5.2

[[node-effectiveAgentProxy]]
effectiveAgentProxy => <<class-node>>::
Object that is effective agent proxy or NULL.

[[node-effectiveIcmpProxy]]
effectiveIcmpProxy => <<class-node>>::
Object that is effective ICMP proxy or NULL.

[[node-effectiveSnmpProxy]]
effectiveSnmpProxy => <<class-node>>::
Object that is effective SNMP proxy or NULL.

[[node-etherNetIPAddress]]
etherNetIPAddress => <<class-inetaddress>>::
Effective address used for EtherNet/IP communication: EtherNet/IP address configured in node properties, or node's primary IP address if not configured.
+
[.version-added]
Added in version 6.1

[[node-flagsAttr]]
flags: int::
Bit mask of <<node-flags>>.

[[node-hardwareComponents]]
hardwareComponents: <<class-array,Array>>::
Returns an array of available hardware of class <<class-hardwarecomponent>>.

[[node-hardwareId]]
hardwareId: <<class-string,String>>::
Nodes' unique hardware id

[[node-hasAgentIfXCounters]]
hasAgentIfXCounters: bool::
`TRUE` if agent supports 64-bit interface counters.

[[node-hasEntityMIB]]
hasEntityMIB: bool::
`TRUE` if supports ENTITY-MIB.

[[node-hasServiceManager]]
hasServiceManager: bool::
`TRUE` if node has service manager accessible via NetXMS agent.

[[node-hasIfXTable]]
hasIfXTable: bool::
`TRUE` if supports ifXTable via SNMP (64-bit counters for interface statistics).

[[node-hasUserAgent]]
hasUserAgent: bool::
`TRUE` if has user agent

[[node-hasVLANs]]
hasVLANs: bool::
`TRUE` if VLAN information available.

[[node-hasWinPDH]]
hasWinPDH: bool::
`TRUE` if node supports Windows PDH parameters.

[[node-hypervisorInfo]]
hypervisorInfo: <<class-string,String>>::
Additional information about hypervisor for this node.

[[node-hypervisorType]]
hypervisorType: <<class-string,String>>::
Hypervisor type as `string` (usually hypervisor vendor or product name, like VMWare or XEN).

[[node-icmpAverageRTT]]
icmpAverageRTT: int::
ICMP average response time for primary address. Will return null if no information.

[[node-icmpLastRTT]]
icmpLastRTT: int::
ICMP last response time for primary address. Will return null if no information.

[[node-icmpMaxRTT]]
icmpMaxRTT: int::
ICMP maximal response time for primary address. Will return null if no information.

[[node-icmpMinRTT]]
icmpMinRTT: int::
ICMP minimal response time for primary address. Will return null if no information.

[[node-icmpPacketLoss]]
icmpPacketLoss: int::
ICMP packet loss for primary address. Will return null if no information.

[[node-icmpProxy]]
icmpProxy => <<class-node>>::
Object that is set as ICMP proxy in object properties or NULL.

[[node-interfaces]]
interfaces: <<class-array,Array>>::
Array with <<class-interface>> objects, that are under this node. First object placed at index 0.

[[node-is802_1x]]
is802_1x: bool::
`TRUE` if node supports 802.1x. Equivalent of `isPAE`.

[[node-isAgent]]
isAgent: bool::
`TRUE` if NetXMS agent detected on node

[[node-isAgentRestartPending]]
isAgentRestartPending: bool::
`TRUE` if NetXMS agent on this node reported that its restart is pending (for example, after agent upgrade or policy change).
+
[.version-added]
Added in version 6.1

[[node-isBridge]]
isBridge: bool::
`TRUE` if node is a bridge

[[node-isCDP]]
isCDP: bool::
`TRUE` if node supports CDP (Cisco Discovery Protocol)

[[node-isDecommissioned]]
isDecommissioned: bool::
`TRUE` if node is decommissioned.

[[node-isEtherNetIP]]
isEtherNetIP: bool::
`TRUE` if node supports EtherNet/IP (Industrial Protocol)

[[node-isExternalGateway]]
isExternalGateway: bool::
`TRUE` if node is an external gateway (remotely managed node). Alias: `isRemotelyManaged`.

[[node-isInMaintenanceMode]]
isInMaintenanceMode: bool::
`TRUE` if node is in maintenance mode

[[node-isLLDP]]
isLLDP: bool::
`TRUE` if node supports LLDP (Link Layer Discovery Protocol)

[[node-isLocalManagement]]
isLocalManagement: bool::
`TRUE` if node is a local management server (NetXMS server)

[[node-isLocalMgmt]]
isLocalMgmt: bool::
`TRUE` if node is a local management server (NetXMS server)

[[node-isModbusTCP]]
isModbusTCP: bool::
`TRUE` if node supports Modbus TCP

[[node-isNativeAgent]]
isNativeAgent: bool::
`TRUE` if node is a native agent.

[[node-isNDP]]
isNDP: bool::
`TRUE` if node supports OSPF/NDP. Equivalent of `isOSPF`.

[[node-isNETCONF]]
isNETCONF: bool::
`TRUE` if node supports NETCONF protocol.
+
[.version-added]
Added in version 7.0

[[node-isObserved]]
isObserved: bool::
`TRUE` if this node is matched as a host by at least one <<class-observationpoint>> of a <<class-trafficobserver>>.
+
[.version-added]
Added in version 7.0

[[node-isOSPF]]
isOSPF: bool::
`TRUE` if node supports OSPF/NDP. Equivalent of `isNDP`.

[[node-isPAE]]
isPAE: bool::
`TRUE` if node supports 802.1x. Equivalent of `is802_1x`.

[[node-isPrinter]]
isPrinter: bool::
`TRUE` if node is a printer

[[node-isProfiNet]]
isProfiNet: bool::
`TRUE` if node supports PROFINET (Process Field Network)

[[node-isRouter]]
isRouter: bool::
`TRUE` if node is a router (has IP forwarding enabled)

[[node-isSMCLP]]
isSMCLP: bool::
`TRUE` if node supports SMCLP (Server Management Command Line Protocol)

[[node-isSNMP]]
isSNMP: bool::
`TRUE` if SNMP agent detected on node

[[node-isSONMP]]
isSONMP: bool::
`TRUE` if node supports SONMP/NDP (Synoptics/Nortel Discovery Protocol)

[[node-isSSH]]
isSSH: bool::
`TRUE` if SSH is connection available

[[node-isSSHCommandChannelAvailable]]
isSSHCommandChannelAvailable: bool::
`TRUE` if SSH command channel can be established to this node.

[[node-isSSHInteractiveChannelAvailable]]
isSSHInteractiveChannelAvailable: bool::
`TRUE` if SSH interactive (shell) channel can be established to this node.

[[node-isSTP]]
isSTP: bool::
`TRUE` if node supports STP (Spanning Tree Protocol)

[[node-isVirtual]]
isVirtual: bool::
`TRUE` if node is virtual

[[node-isVRRP]]
isVRRP: bool::
`TRUE` if VRRP supported.

[[node-isWirelessAP]]
isWirelessAP: bool::
`TRUE` if node is wireless access point

[[node-isWirelessController]]
isWirelessController: bool::
`TRUE` if node is wireless controller

[[node-lastAgentCommTime]]
lastAgentCommTime: int::
Unix timestamp of last time when communication with agent was

[[node-modbusProxy]]
modbusProxy => <<class-node>>::
MODBUS proxy object

[[node-modbusProxyId]]
modbusProxyId: int::
MODBUS proxy object id

[[node-modbusTCPPort]]
modbusTCPPort: int::
MODBUS TCP port

[[node-modbusUnitId]]
modbusUnitId: int::
MODBUS unit id

[[node-netconfCapabilities]]
netconfCapabilities: <<class-array,Array>>::
NETCONF capabilities announced by the device during last configuration poll, as array of capability URI strings. Empty array if node does not support NETCONF.
+
[.version-added]
Added in version 7.0

[[node-netconfPort]]
netconfPort: int::
TCP port used for NETCONF connections.
+
[.version-added]
Added in version 7.0

[[node-netconfProxy]]
netconfProxy => <<class-node>>::
Object that is set as NETCONF proxy in node properties or `null`.
+
[.version-added]
Added in version 7.0

[[node-netconfProxyId]]
netconfProxyId: int::
ID of the object that is set as NETCONF proxy in node properties, or `0` if not set.
+
[.version-added]
Added in version 7.0

[[node-networkPathCheckResult]]
networkPathCheckResult => <<class-networkpathcheckresult>>::
Node network path

[[node-nodeSubType]]
nodeSubType: <<class-string,String>>::
Node sub type

[[class-node-types,Node::types]]
nodeType: int::
Node type <<node-types>>

[[node-observationPoints]]
observationPoints: <<class-array,Array>>::
Array of <<class-observationpoint>> objects at which this node was observed. Empty array if the node is not observed.
+
[.version-added]
Added in version 7.0

[[node-ospfAreas]]
ospfAreas => <<class-ospfarea>>::
Returns <<class-ospfarea>> class object

[[node-ospfNeighbors]]
ospfNeighbors: <<class-array,Array>>::
Array with <<class-ospfarea>> class objects

[[node-ospfRouterId]]
ospfRouterId: <<class-string,String>>::
OSPF router ID

[[node-physicalContainer]]
physicalContainer => Object::
Physical container object: Rack <<class-netobj>> or Chassis <<class-chassis>>

[[node-physicalContainerId]]
physicalContainerId: int::
Physical container object id (Rack or Chassis)

[[node-platformName]]
platformName: <<class-string,String>>::
Platform name reported by NetXMS agent

[[node-pollCountForStatusChange]]
pollCountForStatusChange: int::
Number of polls required before registering a status change.

[[node-primaryHostName]]
primaryHostName: <<class-string,String>>::
Primary host name

[[node-productCode]]
productCode: <<class-string,String>>::
Hardware system property - product code

[[node-productName]]
productName: <<class-string,String>>::
Hardware system property - product name

[[node-productVersion]]
productVersion: <<class-string,String>>::
Hardware system property - product version

[[node-rack]]
rack => <<class-netobj>>::
Will return Rack object as <<class-netobj>> if node is added in rack

[[node-rackHeight]]
rackHeight: int::
Object height in rack

[[node-rackId]]
rackId: int::
Will return Rack id if node is added in rack

[[node-rackPosition]]
rackPosition: int::
Object position in rack

[[node-runtimeFlags]]
runtimeFlags: int::
Bit mask of <<node-runtime-flags>>, `uint32`.

[[node-serialNumber]]
serialNumber: <<class-string,String>>::
Serial number from hardware system property

[[node-snmpAgentNames]]
snmpAgentNames: <<class-array,Array>>::
Names of additional SNMP agents configured on this node, as array of strings. Use <<node-createSNMPTransportForAgent,createSNMPTransportForAgent()>> to communicate with a specific additional agent.
+
[.version-added]
Added in version 7.0

[[node-snmpOID]]
snmpOID: <<class-string,String>>::
SNMP object identifier (result of `1.3.6.1.2.1.1.2.0` request)

[[node-snmpProxy]]
snmpProxy => <<class-node>>::
Object that is set as SNMP proxy in object properties or NULL.

[[node-snmpProxyId]]
snmpProxyId: int::
SNMP proxy object id.

[[node-snmpSysContact]]
snmpSysContact: <<class-string,String>>::
SNMP system contact (result of `1.3.6.1.2.1.1.4.0` request)

[[node-snmpSysLocation]]
snmpSysLocation: <<class-string,String>>::
SNMP system location (result of `1.3.6.1.2.1.1.6.0` request)

[[node-snmpSysName]]
snmpSysName: <<class-string,String>>::
SNMP system name (result of `1.3.6.1.2.1.1.5.0` request)

[[node-snmpVersion]]
snmpVersion: int::
Configured SNMP version:
+
* 0: SNMP version 1
* 1: SNMP version 2c
* 2: SNMP version 3

[[node-softwarePackages]]
softwarePackages: <<class-array,Array>>::
Returns array of <<class-softwarepackage>> class objects

[[node-sysDescription]]
sysDescription: <<class-string,String>>::
System description (value of `System.Uname` for nodes with agents or `1.3.6.1.2.1.1.1.0` for SNMP nodes)

[[node-tunnel]]
tunnel => <<class-tunnel>>::
Tunnel object f this node or NULL if there is no tunnel active and bound to this node.

[[node-vendor]]
vendor: <<class-string,String>>::
Hardware vendor information

[[node-vlans]]
vlans: <<class-array,Array>>::
Array with object <<class-vlan>> objects (`null` if there are no VLANs)

[[node-wirelessDomain]]
wirelessDomain => <<class-wirelessdomain>>::
Wireless domain of this node

[[node-wirelessDomainId]]
wirelessDomainId: int::
Wireless domain id of this node

[[node-wirelessStations]]
wirelessStations: <<class-array,Array>>::
Array with object <<class-wirelessstation>> objects (`null` if there are no Wireless Stations)

[[node-zone]]
zone => <<class-zone>>::
<<class-zone>> object (`null` if zoning is disabled)

[[node-zoneProxyAssignments]]
zoneProxyAssignments: int::
Number of objects where this node is selected as either primary or backup zone proxy (`0` if zoning is disabled or this node is not a zone proxy).

[[node-zoneProxyStatus]]
zoneProxyStatus: bool::
Status of this node as zone proxy (`true` if active).

[[node-zoneUIN]]
zoneUIN: int::
This node zone UIN
--

==== Instance methods

include::_shared/netobj-methods.adoc[]

include::_shared/datacollectiontarget-methods.adoc[]

[.method-list]
--
[[node-callWebService]]
callWebService(webSvcName, requestType, ...) => <<class-webserviceresponse>>::
Finds web service by name, makes request and returns the <<class-webserviceresponse>> object.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|webSvcName|String|Web service name.
|requestType|String|Request type. One of: GET, DELETE, PATCH, POST, PUT
|data|String or <<class-jsonobject>> |Depending on request type data might be required and provided as a string or JSON object. GET and DELETE requests do not allow data, PATCH, POST, PUT require data.
|contentType|String|Optional parameter, for PATCH, POST and PUT requests only. Type of provided data that will be set to "Content-Type" header of request. Default type is "application/json".
|acceptCached|Boolean|Optional named parameter. If True, response stored in agent's cache will be used. Default: false.
|...|Strings|Optional additional parameter(s) that will be used in web service definition to expand %1, %2... macro.
|===
+
.Return
Instance of <<class-webserviceresponse>> with request result.
+
[.source]
....
// Valid example when request fails because there is no connection to agent
result = $node.callWebService("Web service name", "GET", "additional/ult/path");
println(result.success); // "false"
println(result.errorMessage); // "No connection with agent"
println(result.agentErrorCode); // "0"
println(result.httpResponseCode); // "0"
println(result.document); // ""

// Successful post request
result = $node.callWebService("Web service name", "POST", "{ \"id\":10 }", "application/json", "additional/ult/path");
println(result.success); // "true"
println(result.errorMessage); // ""
println(result.agentErrorCode); // "0"
println(result.httpResponseCode); // "200"
println(result.document); // ""

// Get requests
result = $node.callWebService("Web service name", "GET", "additional", "parameters");
result = $node.callWebService("Web service name", "GET", acceptCached: true, "additional", "parameters);
....

[[node-clearSNMPTrapCredentials]]
clearSNMPTrapCredentials(): bool::
Remove separate SNMP trap credentials set with <<node-setSNMPTrapCommunity,setSNMPTrapCommunity()>> or <<node-setSNMPTrapUSMCredentials,setSNMPTrapUSMCredentials()>>. After this call, incoming traps are validated against the node's polling credentials again.
+
.Return
`TRUE` on success, `FALSE` if caller has no modify access to the node.
+
[.version-added]
Added in version 6.2

[[node-createSNMPTransport]]
createSNMPTransport(port, community, context, failIfUnreachable) => <<class-snmptransport>>::
Create SNMP transport object of class <<class-snmptransport>> with communication settings defined on the node. It is possible to specify a community string but only community strings listed in Network Credentials will be accepted.
Creation of SNMP transport is a preparatory operation enabling subsequent sending of SNMP requests to node.
However, creation of SNMP transport does not guarantee that the node is accessible.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|port|Integer|Optional parameter with port.
|community|String|Optional parameter with community string.
|context|String|Optional parameter with context.
|failIfUnreachable|String|Optional parameter, is provided and is true, check that SNMP is reachable on the node before creating SNMP transport.
|===
+
[.version-added]
Added in version 4.4.1: `failIfUnreachable` parameter
+
.Return
Instance of <<class-snmptransport>> or `null` if failed or node was not reacheable when failIfUnreachable was set to true.

[[node-createSNMPTransportForAgent]]
createSNMPTransportForAgent(name, failIfUnreachable) => <<class-snmptransport>>::
Create SNMP transport object of class <<class-snmptransport>> for an additional SNMP agent configured on this node. Additional agents are selected by name; available names are listed in <<node-snmpAgentNames,snmpAgentNames>> attribute.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|name|String|Name of the additional SNMP agent.
|failIfUnreachable|Boolean|Optional parameter, if provided and is true, check that node is reachable before creating SNMP transport.
|===
+
.Return
Instance of <<class-snmptransport>> or `null` if agent with given name is not configured, caller has no SNMP read access to the node, or node is unreachable and `failIfUnreachable` is true.
+
[.version-added]
Added in version 7.0
+
[.source]
....
transport = $node.createSNMPTransportForAgent("ups");
if (transport != null)
{
   v = transport.getValue(".1.3.6.1.2.1.33.1.2.4.0");  // upsEstimatedMinutesRemaining
   println("Minutes remaining: " .. v);
}
....

[[node-enable8021xStatusPolling]]
enable8021xStatusPolling(flag): void::
Enable or disable 802.1x port state checking during status polls.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|flag|Boolean|If 802.1x port state checking should be enabled.
|===

[[node-enableAgent]]
enableAgent(flag): void::
Enable or disable usage of NetXMS agent for all polls.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|flag|Boolean|If agent usage should be enabled.
|===

[[node-enableDiscoveryPolling]]
enableDiscoveryPolling(flag): void::
Enable or disable discovery polling.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|flag|Boolean|If discovery polling should be enabled.
|===

[[node-enableEtherNetIP]]
enableEtherNetIP(flag): void::
Enable or disable usage of EtherNet/IP for polls.
+
.Parameters
[cols="1,1a,3a" grid="none", frame="none"]
|===
|flag|Boolean|If EtherNet/IP should be enabled.
|===

[[node-enableIcmp]]
enableIcmp(flag): void::
Enable or disable usage of ICMP pings for status polls.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|flag|Boolean|If ICMP pings should be enabled.
|===

[[node-enableModbusTcp]]
enableModbusTcp(flag): void::
Enable or disable usage of Modbus TCP for polls.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|flag|Boolean|If Modbus TCP should be enabled.
|===

[[node-enableNetconf]]
enableNetconf(flag): void::
Enable or disable usage of NETCONF protocol.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|flag|Boolean|If NETCONF communication should be enabled.
|===
+
[.version-added]
Added in version 7.0

[[node-enablePrimaryIPPing]]
enablePrimaryIPPing(flag): void::
Enable or disable usage of ICMP ping for primary IP.
+
.Parameters
[cols="1,1a,3a" grid="none", frame="none"]
|===
|flag|Boolean|If primary IP ping should be enabled.
|===

[[node-enableRoutingTablePolling]]
enableRoutingTablePolling(flag): void::
Enable or disable routing table polling.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|flag|Boolean|If routing table polls should be enabled.
|===

[[node-enableSnmp]]
enableSnmp(flag): void::
Enable or disable usage of SNMP for all polls.
+
.Parameters
[cols="1,1a,3a" grid="none", frame="none"]
|===
|flag|Boolean|If SNMP communication should be enabled.
|===

[[node-enableSmclpPropertyPolling]]
enableSmclpPropertyPolling(flag): void::
Enable or disable reading of SM-CLP available properties metadata during
configuration poll.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|flag|Boolean|If 802.1x port state checking should be enabled.
|===

[[node-enableSsh]]
enableSsh(flag): void::
Enable or disable usage of SSH
+
.Parameters
[cols="1,1a,3a" grid="none", frame="none"]
|===
|flag|Boolean|If SSH communication should be enabled.
|===

[[node-enableTopologyPolling]]
enableTopologyPolling(flag): void::
Enable or disable topology polling.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|flag|Boolean|If topology polls should be enabled.
|===

[[node-enableVnc]]
enableVnc(flag): void::
Enable or disable VNC detection on the node during configuration poll.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|flag|Boolean|If VNC detection should be enabled.
|===

[[node-enableWinPerfCountersCache]]
enableWinPerfCountersCache(flag): void::
Enable or disable reading of Windows performance counters metadata during
configuration poll.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|flag|Boolean|If reading of Windows performance counters metadata should be enabled.
|===

[[node-executeAgentCommand]]
executeAgentCommand(command, ...): bool::
Execute agent command (action) on node.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|command|String|Command to be executed.
|...|String|Optional arguments for command
|===
+
.Return
True if command execution is successful or false otherwise.

[[node-executeAgentCommandWithOutput]]
executeAgentCommandWithOutput(command, ...): <<class-string,String>>::
Execute agent command (action) on node and return it's output.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|command|String|Command to be executed.
|...|String|Optional arguments for command
|===
+
.Return
String with output of the command or null on failure.

[[node-executeSSHCommand]]
executeSSHCommand(command) => array::
Execute SSH command on node.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|command|String|Command to be executed.
|===
+
.Return
Array of strings with output of the command or null on failure.

[[class-node-getblockedports,Node::getBlockedPorts()]]
getBlockedPorts(protocol): <<class-array,Array>>::
Get array of blocked ports for the specified protocol.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|protocol|String|Protocol type - "tcp" or "udp"
|===
+
.Return
Array of integers representing blocked port numbers.
+
[.source]
....
blockedTcpPorts = $node->getBlockedPorts("tcp");
for (port : blockedTcpPorts) {
    println("Blocked TCP port: " .. port);
}
....

[[class-node-getinstalledpackages,Node::getInstalledPackages()]]
getInstalledPackages(filter, useRegex): <<class-array,Array>>::
Get array of installed packages on the node.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|filter|String|Optional package name filter
|useRegex|Boolean|Optional. If TRUE, use regex matching for filter. Default is FALSE
|===
+
.Return
Array of installed packages on the node.
+
[.source]
....
// Get all packages
packages = $node->getInstalledPackages();

// Get packages matching filter
packages = $node->getInstalledPackages("netxms", false);
....

[[class-node-getinterface,Node::getInterface()]]
getInterface(ifIdentifier) => <<class-interface>>::
Get interface object by index, MAC address or name. If
name is number method will assume that it is index.
+
.Parameters
[cols="1,1a,3a" grid="none", frame="none"]
|===
|ifIdentifier|Integer or String|Index, MAC address or name of interface.
|===
+
.Return
<<class-interface>> object.
+
[.source]
....
println($node.getInterface("00:14:22:04:25:37").name); // Will print "wlp4s0"
println($node.getInterface(3).name); // Will print "wlp4s0"
println($node.getInterface("wlp4s0").name); // Will print "wlp4s0"
....

[[class-node-getinterfacebyindex,Node::getInterfaceByIndex()]]
getInterfaceByIndex(ifIndex) => <<class-interface>>::
Get interface object by index.
+
.Parameters
[cols="1,1a,3a" grid="none", frame="none"]
|===
|ifIndex|Integer|Index of interface.
|===
+
.Return
<<class-interface>> object.
+
[.source]
....
println($node.getInterfaceByIndex(3).name); // Will print "wlp4s0"
....

[[class-node-getinterfacebymacaddress,Node::getInterfaceByMACAddress()]]
getInterfaceByMACAddress(ifMACAddr) => <<class-interface>>::
Get interface object by MAC address.
+
.Parameters
[cols="1,1a,3a" grid="none", frame="none"]
|===
|ifMACAddr|String|MAC address of interface.
|===
+
.Return
<<class-interface>> object.
+
[.source]
....
println($node.getInterfaceByMACAddress("00:14:22:04:25:37").name); // Will print "wlp4s0"
....

[[class-node-getinterfacebyname,Node::getInterfaceByName()]]
getInterfaceByName(IfName) => <<class-interface>>::
Get interface object by name.
+
.Parameters
[cols="1,1a,3a" grid="none", frame="none"]
|===
|IfName|String|Name of interface.
|===
+
.Return
<<class-interface>> object.
+
[.source]
....
println($node.getInterfaceByName("wlp4s0").name); // Will print "wlp4s0"
....

[[node-getInterfaceName]]
getInterfaceName(ifIndex): <<class-string,String>>::
Get interface name by index.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|ifIndex|Integer|Index of interface.
|===

[[node-getRunningConfig]]
getRunningConfig(): <<class-string,String>>::
Retrieve running configuration of the device using its network device driver. Only works for nodes whose driver supports configuration backup (see `NodeCapability::RegisteredForBackup`).
+
.Return
Device configuration as string, or `null` if caller has no control access to the node, driver does not support configuration backup, or retrieval failed.
+
[.version-added]
Added in version 6.1
+
[.source]
....
config = $node.getRunningConfig();
if (config != null)
{
   f = IO::OpenFile("/var/backup/" .. $node.name .. ".cfg", "w");
   if (f != null)
   {
      f.write(config);
      f.close();
   }
}
....

[[node-getStartupConfig]]
getStartupConfig(): <<class-string,String>>::
Retrieve startup configuration of the device using its network device driver. Only works for nodes whose driver supports configuration backup (see `NodeCapability::RegisteredForBackup`).
+
.Return
Device configuration as string, or `null` if caller has no control access to the node, driver does not support configuration backup, or retrieval failed.
+
[.version-added]
Added in version 6.1

[[class-node-isportblocked,Node::isPortBlocked()]]
isPortBlocked(port, protocol): bool::
Check if a specific port is blocked for the given protocol.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|port|Integer|Port number (1-65535)
|protocol|String|Protocol - "tcp" or "udp"
|===
+
.Return
TRUE if the port is blocked, FALSE otherwise.
+
[.source]
....
if ($node->isPortBlocked(22, "tcp")) {
    println("SSH port is blocked");
}
....

[[node-getWebService]]
getWebService(webSvcName) => <<class-webservice>>::
Get web service object by name.
+
.Parameters
[cols="1,1a,3a" grid="none", frame="none"]
|===
|webSvcName|String|Name of interface.
|===
+
.Return
<<class-webservice>> object.
+
[.source]
....
webSvc = $node.getWebService("webSvcName");
....

[[node-openNETCONFSession]]
openNETCONFSession() => <<class-netconfsession>>::
Open NETCONF session to this node. Operations are executed through the node's effective NETCONF proxy; see <<class-netconfsession>> for details and limitations.
+
.Return
<<class-netconfsession>> object, or `null` if caller has no control access to the node.
+
[.version-added]
Added in version 7.0
+
[.source]
....
session = $node.openNETCONFSession();
if (session != null)
{
   hostname = session.get("<system xmlns=\"urn:ietf:params:xml:ns:yang:ietf-system\"><hostname/></system>");
   println(hostname);
}
....

[[class-node-opensshsession,Node::openSSHSession()]]
openSSHSession(user, password, keyId) => <<class-sshsession>>::
Open an interactive SSH session to this node.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|user|String|Optional SSH login username
|password|String|Optional SSH password
|keyId|Integer|Optional SSH key identifier
|===
+
.Return
<<class-sshsession>> object on success, or NULL on failure.
+
[.source]
....
session = $node->openSSHSession();
if (session != null && session->connected) {
    output = session->execute("ls -la");
    for (line : output) {
        println(line);
    }
    session->close();
}
....

[[class-node-readagentlist,Node::readAgentList()]]
readAgentList(name): <<class-array,Array>>::
Request agent list directly from agent on given node. Returns array of strings or `NULL` if failed.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|name|String|List name.
|===

[[class-node-readagentparameter,Node::readAgentParameter()]]
readAgentParameter(name): <<class-string,String>>::
Request metric directly from agent on given node. Returns `NULL` if failed.
+
.Parameters
[cols="1,1a,3a" grid="none", frame="none"]
|===
|name|String|Metric name.
|===

[[class-node-readagenttable,Node::readAgentTable()]]
readAgentTable(name) => <<class-table>>::
Requests agent table directly from agent on given node. Returns <<class-table>> or `NULL` if failed.
+
.Parameters
[cols="1,1a,3a" grid="none", frame="none"]
|===
|name|String|Table name.
|===

[[class-node-readdriverparameter,Node::readDriverParameter()]]
readDriverParameter(name): <<class-string,String>>::
Request driver-specific metric from network device driver (e.g. Rital).
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|name|String|List name.
|===

[[node-readInternalParameter]]
readInternalParameter(name): <<class-string,String>>::
Read internal parameter
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|name|String|Parameter name
|===
+
.Return
Value of requested internal parameter

[[node-readInternalTable]]
readInternalTable(name) => <<class-table>>::
Read internal table
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|name|String|Table name
|===
+
.Return
Value of requested internal table of class <<class-table>>

[[class-node-readWebServiceList,Node::readWebServiceList()]]
readWebServiceList(name): <<class-array,Array>>::
Read list from node using web service
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|name|String|Name is expected in form service:path or service(arguments):path
|===
+
.Return
Array with string with web service instances

[[class-node-readWebServiceParameter,Node::readWebServiceParameter()]]
readWebServiceParameter(name): <<class-string,String>>::
Read from node web service parameter
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|name|String|Name is expected in form service:path or service(arguments):path
|===
+
.Return
String with result read from web service

[[node-setExpectedCapabilities]]
setExpectedCapabilities(capabilities): void::
Set the expected capabilities for this node. Expected capabilities prevent automatic removal of protocol capability flags when a protocol becomes temporarily unreachable. Normally, if a protocol (e.g., SNMP or agent) fails beyond the capability expiration time, its capability flag is cleared. Setting expected capabilities overrides this behavior, keeping the specified capabilities persistent regardless of connectivity status.
+
Only the following <<const-node-capability>> flags are accepted (other bits are masked out):
+
- `NodeCapability::Agent` (0x0000000002)
- `NodeCapability::SNMP` (0x0000000001)
- `NodeCapability::SSH` (0x0000000080)
- `NodeCapability::EtherNetIP` (0x0000800000)
- `NodeCapability::ModbusTCP` (0x0001000000)
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|capabilities|Integer|Bitmask of <<const-node-capability>> flags.
|===
+
.Return
void
+
[.source]
....
// Ensure SNMP and agent capabilities are never automatically removed
$node.setExpectedCapabilities(NodeCapability::SNMP | NodeCapability::Agent);

// Protect all supported protocol capabilities
$node.setExpectedCapabilities(
    NodeCapability::Agent | NodeCapability::SNMP |
    NodeCapability::SSH | NodeCapability::EtherNetIP |
    NodeCapability::ModbusTCP
);

// Clear expected capabilities (allow normal automatic removal)
$node.setExpectedCapabilities(0);
....
+
[.version-added]
Added in version 5.2

[[node-setIfXTableUsageMode]]
setIfXTableUsageMode(mode): void::
Set ifXTable usage mode 0 - use default, 1 - enable, 2 - disable
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|mode|Integer|usage mode
|===

[[node-setPollCountForStatusChange]]
setPollCountForStatusChange(count): void::
Set the required number of polls before registering a status change.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|count|Integer|Number of polls (must be >= 0) or 0 to use system default.
|===

[[node-setProductCode]]
setProductCode(code): void::
Set product code of the node. Value set this way overrides value detected during configuration poll.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|code|String|New product code.
|===
+
[.version-added]
Added in version 6.1

[[node-setProductName]]
setProductName(name): void::
Set product name of the node. Value set this way overrides value detected during configuration poll.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|name|String|New product name.
|===
+
[.version-added]
Added in version 6.1

[[node-setProductVersion]]
setProductVersion(version): void::
Set product version of the node. Value set this way overrides value detected during configuration poll.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|version|String|New product version.
|===
+
[.version-added]
Added in version 6.1

[[node-setSNMPCommunity]]
setSNMPCommunity(community): bool::
Set SNMP community string used for polling this node (SNMP version 1 and 2c).
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|community|String|Community string.
|===
+
.Return
`TRUE` on success, `FALSE` if caller has no modify access to the node.
+
[.version-added]
Added in version 6.2

[[node-setSNMPTrapCommunity]]
setSNMPTrapCommunity(community): bool::
Set separate community string for validating SNMP traps received from this node (SNMP version 1 and 2c). If not set, traps are validated against polling credentials. Use <<node-clearSNMPTrapCredentials,clearSNMPTrapCredentials()>> to remove it.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|community|String|Community string.
|===
+
.Return
`TRUE` on success, `FALSE` if caller has no modify access to the node.
+
[.version-added]
Added in version 6.2

[[node-setSNMPTrapUSMCredentials]]
setSNMPTrapUSMCredentials(userName, authPassword, privPassword, authMethod, privMethod): bool::
Set separate SNMP version 3 USM credentials for validating SNMP traps received from this node. If not set, traps are validated against polling credentials. Use <<node-clearSNMPTrapCredentials,clearSNMPTrapCredentials()>> to remove them.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|userName|String|USM user name.
|authPassword|String|Authentication password.
|privPassword|String|Privacy (encryption) password.
|authMethod|String or Integer|Authentication method: `"none"`, `"md5"`, `"sha1"`, `"sha224"`, `"sha256"`, `"sha384"`, `"sha512"` (case-insensitive), or numeric value 0-6 in the same order.
|privMethod|String or Integer|Privacy method: `"none"`, `"des"`, `"aes-128"`, `"aes-192"`, `"aes-256"` (case-insensitive), or numeric value 0-4 in the same order.
|===
+
.Return
`TRUE` on success, `FALSE` if caller has no modify access to the node or authentication or privacy method is invalid.
+
[.version-added]
Added in version 6.2

[[node-setSNMPUSMCredentials]]
setSNMPUSMCredentials(userName, authPassword, privPassword, authMethod, privMethod): bool::
Set SNMP version 3 USM credentials used for polling this node. Node's SNMP version is switched to 3.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|userName|String|USM user name.
|authPassword|String|Authentication password.
|privPassword|String|Privacy (encryption) password.
|authMethod|String or Integer|Authentication method: `"none"`, `"md5"`, `"sha1"`, `"sha224"`, `"sha256"`, `"sha384"`, `"sha512"` (case-insensitive), or numeric value 0-6 in the same order.
|privMethod|String or Integer|Privacy method: `"none"`, `"des"`, `"aes-128"`, `"aes-192"`, `"aes-256"` (case-insensitive), or numeric value 0-4 in the same order.
|===
+
.Return
`TRUE` on success, `FALSE` if caller has no modify access to the node or authentication or privacy method is invalid.
+
[.version-added]
Added in version 6.2
+
[.source]
....
$node.setSNMPUSMCredentials("monitor", "authpass123", "privpass123", "sha256", "aes-128");
$node.forceConfigurationPoll();
....

[[node-setSNMPVersion]]
setSNMPVersion(version): bool::
Set SNMP version used for polling this node.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|version|String|SNMP version: `"1"`, `"2c"`, or `"3"`.
|===
+
.Return
`TRUE` on success, `FALSE` if caller has no modify access to the node or version string is not recognized.
+
[.version-added]
Added in version 6.2
+
[.source]
....
$node.setSNMPVersion("2c");
$node.setSNMPCommunity("public");
....

[[node-setSerialNumber]]
setSerialNumber(serialNumber): void::
Set serial number of the node. Value set this way overrides value detected during configuration poll.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|serialNumber|String|New serial number.
|===
+
[.version-added]
Added in version 6.1

[[node-setVNCPassword]]
setVNCPassword(password): bool::
Set password used for VNC connections to this node.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|password|String|VNC password.
|===
+
.Return
`TRUE` on success, `FALSE` if caller has no modify access to the node.
+
[.version-added]
Added in version 6.2

[[node-setVendor]]
setVendor(vendor): void::
Set vendor of the node. Value set this way overrides value detected during configuration poll.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|vendor|String|New vendor name.
|===
+
[.version-added]
Added in version 6.1
+
[.source]
....
// Fill hardware information from custom attributes populated by an external inventory system
vendor = $node.customAttributes["inv.vendor"];
if (vendor != null)
{
   $node.setVendor(vendor);
   $node.setProductName($node.customAttributes["inv.model"]);
   $node.setSerialNumber($node.customAttributes["inv.serial"]);
}
....

[[node-traceL2Path]]
traceL2Path(destination) => <<class-networkpath>>::
Trace layer 2 path from this node to the destination node using topology information collected by the server (LLDP, CDP, STP, and other sources). No requests are sent to devices.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|destination|<<class-node>>|Destination node.
|===
+
.Return
<<class-networkpath>> object, or `null` if caller has no read access to the destination node or path cannot be traced. Check <<networkpath-isComplete,isComplete>> attribute of the returned path to see if the destination was reached.
+
[.version-added]
Added in version 6.2
+
[.source]
....
path = $node.traceL2Path(FindObject("core-sw1"));
if ((path != null) && path.isComplete)
{
   for (hop : path.hops)
   {
      if (hop.type == "L2_LINK")
         println(hop.name .. " via " .. hop.interface?.name);
   }
}
....

[[node-traceRoute]]
traceRoute(destination) => <<class-networkpath>>::
Trace layer 3 path from this node to the destination node using routing tables collected by the server. No requests are sent to devices.
+
.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|destination|<<class-node>>|Destination node.
|===
+
.Return
<<class-networkpath>> object, or `null` if caller has no read access to the destination node or path cannot be traced. Check <<networkpath-isComplete,isComplete>> attribute of the returned path to see if the destination was reached.
+
[.version-added]
Added in version 6.2
+
[.source]
....
path = $node.traceRoute(FindObject("core-router"));
if (path != null)
{
   for (hop : path.hops)
      println(hop);
}
....

[[node-wakeUp]]
wakeUp(): bool::
Send Wake-on-LAN magic packet to this node. Interfaces with IPv4 address are tried in order until magic packet is sent successfully; managed interfaces are tried first, then unmanaged ones. See <<interface-wakeup,Interface::wakeUp()>> for details on how the packet is sent.
+
.Return
`TRUE` if magic packet was sent through at least one interface, `FALSE` if no suitable interface was found, packet cannot be sent, or caller has no control access to the node.
+
[.version-added]
Added in version 6.2
+
[.source]
....
// Action script: wake up node that went down
if (!$node.wakeUp())
   println("Cannot send WoL packet to " .. $node.name);
....
--

==== Constants

[[node-flags]]
[cols="1,4a"]
.Node flags
|===
| Description | Value

|DCF_DISABLE_STATUS_POLL
|0x00000001

|DCF_DISABLE_CONF_POLL
|0x00000002

|DCF_DISABLE_DATA_COLLECT
|0x00000004

|DCF_LOCATION_CHANGE_EVENT
|0x00000008

|NF_EXTERNAL_GATEWAY
|0x00010000

|NF_DISABLE_DISCOVERY_POLL
|0x00020000

|NF_DISABLE_TOPOLOGY_POLL
|0x00040000

|NF_DISABLE_SNMP
|0x00080000

|NF_DISABLE_NXCP
|0x00100000

|NF_DISABLE_ICMP
|0x00200000

|NF_FORCE_ENCRYPTION
|0x00400000

|NF_DISABLE_ROUTE_POLL
|0x00800000

|NF_AGENT_OVER_TUNNEL_ONLY
|0x01000000

|NF_SNMP_SETTINGS_LOCKED
|0x02000000

|NF_PING_PRIMARY_IP
|0x04000000

|NF_DISABLE_ETHERNET_IP
|0x08000000

|NF_DISABLE_PERF_COUNT
|0x10000000

|===

[[node-runtime-flags]]
[cols="1,4a"]
.Node runtime flags
|===
| Description | Value

|DCDF_QUEUED_FOR_STATUS_POLL
|0x00000001

|DCDF_QUEUED_FOR_CONFIGURATION_POLL
|0x00000002

|DCDF_QUEUED_FOR_INSTANCE_POLL
|0x00000004

|DCDF_DELETE_IN_PROGRESS
|0x00000008

|DCDF_FORCE_STATUS_POLL
|0x00000010

|DCDF_FORCE_CONFIGURATION_POLL
|0x00000020

|DCDF_CONFIGURATION_POLL_PASSED
|0x00000040

|DCDF_CONFIGURATION_POLL_PENDING
|0x00000080

|NDF_QUEUED_FOR_TOPOLOGY_POLL
|0x00010000

|NDF_QUEUED_FOR_DISCOVERY_POLL
|0x00020000

|NDF_QUEUED_FOR_ROUTE_POLL
|0x00040000

|NDF_RECHECK_CAPABILITIES
|0x00080000

|NDF_NEW_TUNNEL_BIND
|0x00100000

|===

[[node-capabilities]]

Node capability flags are available as <<const-node-capability>> named constants.

[cols="2,1,5a"]
.Node capability flags
|===
| Constant | Value | Description

|NodeCapability::SNMP|0x0000000001|Node supports SNMP
|NodeCapability::Agent|0x0000000002|NetXMS agent detected on the node
|NodeCapability::Bridge|0x0000000004|Node is a network bridge
|NodeCapability::Router|0x0000000008|Node is an IP router
|NodeCapability::LocalManagement|0x0000000010|Node is management server (NetXMS server itself)
|NodeCapability::Printer|0x0000000020|Node is a printer
|NodeCapability::OSPF|0x0000000040|Node supports OSPF
|NodeCapability::SSH|0x0000000080|SSH support
|NodeCapability::CDP|0x0000000100|Cisco Discovery Protocol supported
|NodeCapability::NDP|0x0000000200|NDP (SONMP) support detected (Nortel/Synoptics/Bay Networks topology discovery)
|NodeCapability::LLDP|0x0000000400|Node supports LLDP
|NodeCapability::VRRP|0x0000000800|Node supports VRRP
|NodeCapability::VLAN|0x0000001000|VLAN information available on the node
|NodeCapability::IEEE802_1x|0x0000002000|802.1x support detected
|NodeCapability::STP|0x0000004000|Spanning Tree (IEEE 802.1d) enabled on node
|NodeCapability::EntityMIB|0x0000008000|Node supports ENTITY-MIB
|NodeCapability::IfXTable|0x0000010000|Node supports ifXTable via SNMP (64-bit interface counters)
|NodeCapability::AgentIfXCounters|0x0000020000|Agent supports 64-bit interface counters
|NodeCapability::WindowsPDH|0x0000040000|Node supports Windows PDH parameters
|NodeCapability::WiFiController|0x0000080000|Node is wireless network controller
|NodeCapability::SMCLP|0x0000100000|Node supports SMCLP protocol
|NodeCapability::NewPolicyTypes|0x0000200000|Agent upgraded to new policy type
|NodeCapability::UserAgent|0x0000400000|User agent (desktop support app) is installed
|NodeCapability::EtherNetIP|0x0000800000|EtherNet/IP support
|NodeCapability::ModbusTCP|0x0001000000|Modbus TCP support
|NodeCapability::ProfiNet|0x0002000000|PROFINET support
|NodeCapability::FileManager|0x0004000000|Node has file manager support
|NodeCapability::LLDPv2|0x0008000000|LLDP v2 MIB support
|NodeCapability::EmulatedEntityMIB|0x0010000000|ENTITY MIB support emulated by driver
|NodeCapability::DeviceView|0x0020000000|Device view is supported
|NodeCapability::WiFiAccessPoint|0x0040000000|Node is wireless access point
|NodeCapability::VNC|0x0080000000|Node supports VNC connection from server or zone proxy
|NodeCapability::LocalVNC|0x0100000000|Node supports VNC connection via local agent
|NodeCapability::RegisteredForBackup|0x0200000000|Node is registered for device configuration backup
|NodeCapability::ServiceManager|0x0400000000|Node has service manager accessible via NetXMS agent
|NodeCapability::SSHInteractiveChannel|0x0800000000|Interactive (shell) SSH channel can be established
|NodeCapability::SSHCommandChannel|0x1000000000|SSH command channel can be established

|===


[[node-types]]
[cols="1,5a"]
.Node types
|===
| Value | Description

|0
|Unknown

|1
|Physical

|2
|Virtual

|3
|Controller

|4
|Container

|===

.See also
<<func-findnodeobject>>, <<func-findnodebyipaddress>>, <<func-findnodebysysname>>, <<func-findnodebymacaddress>>, <<func-getallnodes>>, <<class-interface>>, <<class-datacollectiontarget>>
