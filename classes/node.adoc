...
`callWebService(webSwcName, requestType, ...) => <<class-web_service_response>>`::
Finds web service by name, makes request and returns the <<class-web_service_response>> object.

.Parameters
[cols="1,1,3a" grid="none", frame="none"]
|===
|webSwcName|String|Web service name.
|requestType|String|Request type. One of: GET, DELETE, PATCH, POST, PUT
|data|String or <<class-json_object>> |Depending on request type data might be required and provided as a string or json object. GET and DELETE requests do not allow data, others require data.
...
[.source]
....
//Valid example when request fails because there is no connection with agent
