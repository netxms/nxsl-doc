...
[[class-netobj-createCollector,NetObj::createCollector()]]
`createCollector(name) => <<class-collector>>`::
Create new collector child object. Collectors can be created only under <<class-container>>, <<class-collector>> and <<class-serviceroot>>.
